...
-   *We will start this lesson by using the PSYC300_dataset.csv file on
    our ELMS site or in Jupyter.*

## Let's begin by opening/loading dataset

```{r}
dat.work <- read.csv("PSYC300_dataset.csv") # read in data, make sure the file path routes to the appropriate location in ELMS
```
...
-   Here we will do some basic arithmetic using R to create new vectors
...
-   **Here we look at exam scores by each value of statistics anxiety by
    directly referencing the level we are interested in using \[ \] and
    then ==**
...
*Less than/less than or equal to logical operator (\< or \<=). Also
works for greater than/greater than or equal to (\> or \>=).*
